...
## Packing cartridges
To bundle the current Pico-8 workbench into both Pico-8 and Picotron cartridges:

1. Ensure you have `lua` available on your host or inside Picotron.
2. Run `lua pico8/tools/pack_carts.lua` from the repo root.
3. Load the generated carts and run them:
   - Pico-8: `load("pico8/build/zork1.p8")` then type `run`.
   - Picotron: `load("pico8/build/zork1.p64")` then type `run`.

The packer inlines `game_data.lua` into `main.lua` and emits both a Pico-8 `.p8` text cart and a Picotron `.p64` text cart so you can test keyboard and terminal flows.
