...
3. Type commands such as `look`, `n`, `take leaflet`, or `inventory`. The cart echoes your input and updates the world state in-memory.

## Next steps
- Flesh out the room/object data exporter so the full ZIL world can be loaded into Lua tables.
- Expand the verb table (unlocking/closing the window, entering the house, etc.).
